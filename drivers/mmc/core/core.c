...
void mmc_stats(struct work_struct *work)
{
	...
	if ((wtime > 0) || (wtime && (stats_interval == MMC_STATS_LOG_INTERVAL))) {
		#if 0
	...
	if ((rtime > 0) || (rtime && (stats_interval == MMC_STATS_LOG_INTERVAL))) {
		if (host->debug_flag & MMC_DEBUG_FREE_SPACE)
...
/*
SD debounce algorithm
(1) disable interrupt
(2) start 50ms timer
(3) 50ms timer expired, re-enable interrupt wait another 50ms
(4.1) if interrupt still be trigger, then clear timer, re-start the cycle from (1)
(4.2) if the 2’nd 50ms timer expired, then scan the SD card
*/
...
int _mmc_detect_card_removed(struct mmc_host *host)
{
	...
	/* rely on get_cd function to detect card state */
	if (host->ops->get_cd)
		ret = !host->ops->get_cd(host);
	else
		ret = 0;
#if 0
	host->bus_ops->alive(host);
