...
#define PANEL_ID_A32E_TRULY_HX8394F       4
#define PANEL_ID_A32E_MR_TRULY_HX8394D    5
	...
	if (enable) {
		...
		if (pdata->panel_info.panel_id == PANEL_ID_A32E_EVM_TIANMA_HX8394D) {
			...
			msleep(150);
				...
				(pdata->panel_info.panel_id == PANEL_ID_A32E_MR_TRULY_HX8394D)) {
			/* Reset */
			gpio_set_value(ctrl_pdata->rst_gpio, 0);
			usleep_range(1000, 1500);
			gpio_set_value(ctrl_pdata->rst_gpio, 1);
			...
			if (strcmp(htc_get_bootmode(), bootstr))
				...
				PR_DISP_ERR("Recovery mode, discard send boost i2c command");
			msleep(180);
		} else if (pdata->panel_info.panel_id == PANEL_ID_A32E_TRULY_HX8394F) {
			/* Reset */
			gpio_set_value(ctrl_pdata->rst_gpio, 0);
			usleep_range(1000, 1500);
			gpio_set_value(ctrl_pdata->rst_gpio, 1);
