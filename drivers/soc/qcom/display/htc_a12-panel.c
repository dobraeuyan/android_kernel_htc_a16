...
#define PANEL_ID_A12_BITLAND_ILI9806E 1
#define PANEL_ID_A12_TRULY_ILI9806E 2
...
static int htc_a12_regulator_init(struct platform_device *pdev)
{
	int ret = 0;
...
static int htc_a12_regulator_deinit(struct platform_device *pdev)
{
...
void htc_a12_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	if (pdata == NULL) {
	...
	if (enable) {
		...
		if (pdata->panel_info.panel_id == PANEL_ID_A12_BITLAND_ILI9806E) {
			gpio_set_value((ctrl_pdata->rst_gpio), 1);
			usleep(1000);
		} else if (pdata->panel_info.panel_id == PANEL_ID_A12_TRULY_ILI9806E) {
			gpio_set_value((ctrl_pdata->rst_gpio), 1);
			usleep(10000);
		}
	} else {
		if (pdata->panel_info.panel_id == PANEL_ID_A12_BITLAND_ILI9806E ||
			pdata->panel_info.panel_id == PANEL_ID_A12_TRULY_ILI9806E) {
...
static int htc_a12_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	int ret;
...
static struct mdss_dsi_pwrctrl dsi_pwrctrl = {
	.dsi_regulator_init = htc_a12_regulator_init,
	.dsi_regulator_deinit = htc_a12_regulator_deinit,
	.dsi_power_on = htc_a12_panel_power_on,
	.dsi_panel_reset = htc_a12_panel_reset,
};
