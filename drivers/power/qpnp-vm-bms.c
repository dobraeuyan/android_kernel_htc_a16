...
/* check usb temp MPP pin is ready or not */
...
#define BMS_STORE_MAGIC_OFFSET		3104	/*0xC20*/
#define BMS_STORE_SOC_OFFSET		3108	/*0xC24*/
#define BMS_STORE_CURRTIME_OFFSET	3120	/*0xC30*/
...
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	if (time_since_last_change_sec < 0) {
		/* If time re-set, not let it impact soc_changed. */
...
{
/* Do not hold wake lock in low voltage */
#ifndef CONFIG_HTC_BATT_8960
	if (!bms_wake_active(&chip->vbms_lv_wake_source)
...
static int bms_request_irqs(struct qpnp_bms_chip *chip)
{
	...
	if (!test_power_monitor) { /* This irq trigger frequently when use power monitor */
...
/* If kernel RTC is less than 5mins */
/* Treat it as RTC reset by user replace battery */
...
int pm8909_bms_consistence_check(void)
{
	...
	if (!chip->shutdown_soc_invalid
		...
		/* For removable battery, still doing consistent algo unless RTC is reset */
...
static int parse_bms_dt_properties(struct qpnp_bms_chip *chip)
{
	...
	/* Only A13#WL XC or above device support USB overheat algo */
	if (pid_info == 389) {
	...
	pr_debug("force-s3-on-suspend=%d report-charger-eoc=%d disable-bms=%d disable-suspend-on-usb=%d aging_compensation=%d\n",
			...
			chip->dt.cfg_battery_aging_comp);
	pr_debug("use-reported-soc is %d, is-embeded-batt=%d\n",
			chip->dt.cfg_use_reported_soc, chip->dt.is_embeded_batt);
#ifdef CONFIG_HTC_BATT_8960
	pr_debug("pid_info=%d, skuid.pcb_id=%d\n", pid_info, chip->dt.pcb_id);
#endif
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	/* This check need wait charger finish to check temeperature. */
...
static void process_resume_data(struct qpnp_bms_chip *chip)
{
	...
	if (old_ocv != chip->last_ocv_uv) {
		...
		chip->calculated_soc = lookup_soc_ocv(chip,
				chip->last_ocv_uv, batt_temp);
		pr_debug("OCV in sleep SOC=%d, old_ocv=%d, last_ocv_uv=%d\n",
...
/*
 * hctosys.c uses late_initcall to init rtc_hctosys. To get correct UT time,
 * we uses late_initcall to init bms to call pm8916_bms_consistence_check.
 */
