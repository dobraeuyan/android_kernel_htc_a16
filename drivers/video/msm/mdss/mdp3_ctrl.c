...
static int mdp3_ctrl_off(struct msm_fb_data_type *mfd)
{
	...
	if (mdss_fb_is_power_on_lp(mfd)) {
		...
		*/
		...
		/* User space will set backlight 0 when suspend */
#if 0
					...
					struct mdp_display_commit *cmt_data)
{
	...
	mdp_pp.data.csc_cfg_data = htc_set_csc_cfg(&csc_update, panel_info->csc_type);
	if (csc_update) {
		rc = mdp3_csc_config(mdp3_session, &(mdp_pp.data.csc_cfg_data));
		if (rc) {
			pr_err("[DISP] fail to set csc cfg\n");
			return -EINVAL;
		}
	}
					...
					struct mdp_bl_scale_data *data)
{
	...
	if (curr_bl)  /* Skip backlight 0 case */
