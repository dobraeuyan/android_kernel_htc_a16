...
#define CSC_CFG_MV0                     512
#define CSC_CFG_MV1                     0
#define CSC_CFG_MV2                     0
#define CSC_CFG_MV3                     0
#define CSC_CFG_MV4                     512
#define CSC_CFG_MV5                     0
#define CSC_CFG_MV6                     0
#define CSC_CFG_MV7                     0
#define CSC_CFG_MV8                     512
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if ((pinfo->panel_id == 4) || (pinfo->panel_id == 5)) {
		ctrl_pdata->pwm_min = 8;
		ctrl_pdata->pwm_default = 97;
		ctrl_pdata->pwm_max = 255;
		pinfo->camera_blk = 217;
		if (tmous_sku) {
			/* special backlight setting for T-Mobile US SKU */
			ctrl_pdata->pwm_min = 8;
			ctrl_pdata->pwm_default = 97;   /*ToDo, need check with optical team */
			ctrl_pdata->pwm_max = 255;
			pinfo->camera_blk = 217;
		}
	}else if (vzw_sku) {
		/* special backlight setting for Verizon SKU */
	...
	data = of_get_property(np, "htc,mdss-csc-type", NULL);
	if (data) {
		if (!strcmp(data, "color_temperature"))
			pinfo->csc_type = CSC_TYPE_COLOR_TEMPERATURE;
		else if (!strcmp(data, "color_saturation"))
			pinfo->csc_type = CSC_TYPE_COLOR_SATURATION;
		else if (!strcmp(data, "color_saturation+color_hue"))
			pinfo->csc_type = CSC_TYPE_COLOR_SATURATION_AND_HUE;
		else
			pinfo->csc_type = CSC_TYPE_NONE;
	} else {
		pinfo->csc_type = CSC_TYPE_NONE;
	}

	if ((pinfo->csc_type == CSC_TYPE_COLOR_TEMPERATURE) ||
			(pinfo->csc_type == CSC_TYPE_COLOR_SATURATION) ||
			(pinfo->csc_type == CSC_TYPE_COLOR_SATURATION_AND_HUE)) {
		rc = of_property_read_u32_array(np, "htc,mdss-csc-cfg", res, 9);
		pinfo->csc_mv0  = (!rc ? res[0] : CSC_CFG_MV0);
		pinfo->csc_mv4  = (!rc ? res[4] : CSC_CFG_MV4);
		pinfo->csc_mv8  = (!rc ? res[8] : CSC_CFG_MV8);
		if ((pinfo->csc_type == CSC_TYPE_COLOR_SATURATION) ||
				(pinfo->csc_type == CSC_TYPE_COLOR_SATURATION_AND_HUE)) {
			pinfo->csc_mv1  = (!rc ? res[1] : CSC_CFG_MV1);
			pinfo->csc_mv2  = (!rc ? res[2] : CSC_CFG_MV2);
			pinfo->csc_mv3  = (!rc ? res[3] : CSC_CFG_MV3);
			pinfo->csc_mv5  = (!rc ? res[5] : CSC_CFG_MV5);
			pinfo->csc_mv6  = (!rc ? res[6] : CSC_CFG_MV6);
			pinfo->csc_mv7  = (!rc ? res[7] : CSC_CFG_MV7);
		}
	}
...
int __init cmdline_board_cid_check(char *s)
{
	...
	vzw_sku |= !strncmp(s, "VZW__003", 8);
	tmous_sku = !strncmp(s, "T-MOB010", 8);
	return 1;
}
