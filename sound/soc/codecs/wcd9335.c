...
{
	struct snd_soc_codec *codec = w->codec;
	u16 lineout_vol_reg = 0, lineout_mix_vol_reg = 0;	//HTC_AUD
	int ret = 0;
...
{
	struct snd_soc_codec *codec = w->codec;
	u16 boost_path_ctl = 0, boost_path_cfg1 = 0;	//HTC_AUD
	u16 reg = 0, reg_mix = 0; //HTC_AUD
...
static int tasha_codec_enable_spline_src(struct snd_soc_codec *codec,
					 ...
					 int event)
{
	u16 rx_path_cfg_reg = 0;	//HTC_AUD
	u16 rx_path_ctl_reg = 0;	//HTC_AUD
	u16 src_clk_reg = 0;	//HTC_AUD
	int *src_users, count = 0, spl_src = 0;	//HTC_AUD
	struct tasha_priv *tasha;
				...
				int event)
{
	...
	struct snd_soc_codec *codec = w->codec;
	struct tasha_priv *tasha = snd_soc_codec_get_drvdata(codec);
	u16 vbat_path_ctl = 0, vbat_cfg = 0, vbat_path_cfg = 0;	//HTC_AUD
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
	unsigned int val;
	unsigned short look_ahead_dly_reg = 0;	//HTC_AUD
			   ...
			   struct snd_soc_dai *dai)
{
	...
	int ret;
	struct snd_soc_codec *codec = dai->codec;
	int fs_rate = -EINVAL, i2s_bit_mode = 0;	//HTC_AUD
		...
		u32 clk_freq)
{
	struct snd_soc_codec *codec = data;
	struct tasha_priv *tasha;
	u32 cpe_clk_khz = 0, req_freq = 0;	//HTC_AUD
