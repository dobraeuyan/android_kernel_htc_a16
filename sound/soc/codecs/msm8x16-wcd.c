	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (strnstr(w->name, internal1_text, strlen(w->name))) {
			...
			//snd_soc_update_bits(codec, micb_int_reg, 0x10, 0x10); //htc audio - for 8909 HW design
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (strnstr(w->name, internal1_text, strlen(w->name))) {
			...
			snd_soc_update_bits(codec, micb_int_reg, 0x18, 0x00);	//htc audio
...
static const struct msm8x16_wcd_reg_mask_val
	...
	//{MSM8X16_WCD_A_ANALOG_RX_COM_OCP_CTL, 0xFF, 0x12}, // HTC_AUD_MARK QCT new setting
