				...
				bool is_gapless_mode)
{
	...
	int htc_topo = msm8x16_get_htc_offload_topology();
	...
	//HTC_AUDIO_START
	/* For htc offload popp topology */
	if((ac->io_mode & COMPRESSED_IO) || (ac->io_mode & COMPRESSED_STREAM_IO)) {
		if((htc_topo >= 0x10000000) && (htc_topo <= 0x1000000F) ) {
			pr_info("offload changes postprocopo_id = 0x%x --> 0x%x\n", open.postprocopo_id, htc_topo);
			open.postprocopo_id = htc_topo;
		}
	}
	//HTC_AUDIO_END
