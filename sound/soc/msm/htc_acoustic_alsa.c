...
//		return -EINVAL;
	...
	case _IOC_NR(ACOUSTIC_SET_HTC_OFFLOAD_TOPOLOGY): {
		if(sizeof(s32_value) <= us32_size) {
			memcpy((void*)&s32_value, (void*)buf, sizeof(s32_value));
			D("%s %d: ACOUSTIC_SET_HTC_OFFLOAD_TOPOLOGY 0x%0x\n", __func__, __LINE__, s32_value);


			if (s32_value < 0x10000000 || s32_value > 0x1000000F) {
				E("unsupported topology :0x%x\n", s32_value);
				rc = -EINVAL;
				break;
			}
			if (the_ops->set_htc_offload_topology) {
				the_ops->set_htc_offload_topology(s32_value);
			} else {
				E("unsupported set_htc_offload_topology\n");
				rc = -EINVAL;
				break;
			}
		}
		else {
			E("%s %d: ACOUSTIC_SET_HTC_OFFLOAD_TOPOLOGY error.\n", __func__, __LINE__);
			rc = -EINVAL;
		}
		break;
	}
